...
# 5m ((effect timer + offset4) % 300): level 9 5m effect group
...
{
    # curse leak 2
    execute positioned as @s run function __:curse/effect/curse_leak2

    # apply mob aggression effect
    #!sb @s __mob_aggression = 1
    #!sb @s __mob_aggression_end = global __effect_timer
    scoreboard players add @s __mob_aggression_end 60
}
