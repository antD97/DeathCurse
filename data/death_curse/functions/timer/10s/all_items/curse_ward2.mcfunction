...
# - if __time_alive is 2+, set @s __result1 = number of nearby __time_alive=2+ curse ward 2s, otherwise @s __result1 = 0
...
execute as @s[scores={__time_alive=1}] run function __:timer/10s/all_items/curse_ward2/soon_particle
{
    #!5x
    execute at @s run particle ambient_entity_effect ~ ~0.125 ~ 85 0 0 255 0
}

# if time alive is cooked enough, count close and cooked enough curse ward 2s into global __result1
#!sb global __result1 = 0
execute if score @s __time_alive matches 2.. at @s as @e[type=item,scores={__time_alive=2..},nbt={Item:{id:"minecraft:enchanted_book",tag:{CustomModelData:379301}}},distance=..1] run scoreboard players add global __result1 1
