...
# For every resurrection abomination 2...
# - if on fire & in nether, increment @s __time_alive, otherwise @s __time_alive = 0
# - if __time_alive is 2+, set @s __result1 = number of nearby __time_alive=2+ resurrection abomination 2s, otherwise @s __result1 = 0
...
execute as @s[scores={__time_alive=1}] run function __:timer/10s/all_items/res_abom2/soon_particle
{
    #!5x
    execute at @s run particle ambient_entity_effect ~ ~0.125 ~ 85 0 0 255 0
}

# if time alive is cooked enough, count close and cooked enough resurrection abomination 2s into global __result1
#!sb global __result1 = 0
execute if score @s __time_alive matches 2.. at @s as @e[type=item,scores={__time_alive=2..},nbt={Item:{id:"minecraft:beetroot_soup",tag:{CustomModelData:379307}}},distance=..1] run scoreboard players add global __result1 1
